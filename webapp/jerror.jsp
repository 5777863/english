...
<body>
	<div class='start'>
		...
		<div class='main'>
			<div class='unit'>
				<c:choose>
					<c:when test="${empty sessionScope.errormsg == false}">
						<p class='center'>${sessionScope.errormsg}</p>
					</c:when>
					<c:otherwise>
						<p class='center'>Жаль! Произошла неизвестная ошибка,
							попробуйте позже или обратитесь к администрации ресурса</p>
					</c:otherwise>
				</c:choose>
			</div>
		</div>
